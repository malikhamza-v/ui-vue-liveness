...
import { IlluminationState, FaceMatchState, LivenessErrorState } from '../types'
 ...
 */
function getScaledValueFromRandomSeed(randomSeed: number, min: number, max: number): number {
 ...
 */
export function getOvalBoundingBox(ovalDetails: LivenessOvalDetails): OvalBoundingBox {
 ...
 */
export function getIntersectionOverUnion(box1: BoundingBox, box2: BoundingBox): number {
...
export function drawLivenessOvalInCanvas({
  ...
  if (ctx) {
    ...
    // fill the canvas with a transparent rectangle (invisible)
    ...
    // draw the oval path
    ctx.beginPath()
    ctx.ellipse(flippedCenterX, centerY, width / 2, (height * 1.8) / 2, 0, 0, 2 * Math.PI)

    // add stroke to the oval path (invisible)
...
export function clearOvalCanvas({ canvas }: { canvas: HTMLCanvasElement }): void {
  const ctx = canvas.getContext('2d')
 ...
 */
export function estimateIllumination(videoEl: HTMLVideoElement): IlluminationState | undefined {
...
const isColorSequence = (obj: ColorSequence | undefined): obj is ColorSequence => !!obj

export function getColorsSequencesFromSessionInformation(
  parsedSessionInformation: ParsedSessionInformation,
): ColorSequence[] {
  const colorSequenceFromServerChallenge =
    (parsedSessionInformation.Challenge as FaceMovementAndLightChallenge).ColorSequences ?? []
  const colorSequences: (ColorSequence | undefined)[] = colorSequenceFromServerChallenge.map(
    ({
      FreshnessColor,
  ...
  switch (detectedFaces.length) {
    ...
    case 1: {
      ...
      const calibratedPupilDistance =
        (PUPIL_DISTANCE_WEIGHT * pupilDistance + FACE_HEIGHT_WEIGHT * faceHeight) /
        2 /
