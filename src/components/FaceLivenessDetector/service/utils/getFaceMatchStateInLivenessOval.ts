import type { BoundingBox, Face, LivenessOvalDetails, ParsedSessionInformation } from '../types'
import { FaceMatchState } from '../types'

import { generateBboxFromLandmarks, getIntersectionOverUnion, getOvalBoundingBox } from './liveness'
import {
...
export function getFaceMatchStateInLivenessOval({
  ...
  const { OvalIouThreshold, FaceIouHeightThreshold, FaceIouWidthThreshold, OvalHeightWidthRatio } =
    challengeConfig
  ...
  // Reduce thresholds to be less strict about "too far" detection
  const intersectionThreshold = OvalIouThreshold * 0.6 // 40% more lenient
  const faceDetectionWidthThreshold = ovalDetails.width * (FaceIouWidthThreshold * 0.5) // 50% more lenient
  const faceDetectionHeightThreshold = ovalDetails.height * (FaceIouHeightThreshold * 0.5) // 50% more lenient
