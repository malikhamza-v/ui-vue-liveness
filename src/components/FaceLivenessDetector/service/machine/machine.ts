...
const responseStreamActor = async (callback: StreamActorCallback) => {
  try {
    ...
    if (isInvalidSignatureRegionException(error)) {
      ...
      callback({
        type: isConnectionTimeoutError(error) ? 'CONNECTION_TIMEOUT' : 'SERVER_ERROR',
        data: { error },
      })
    }
  }
...
export const livenessMachine = createMachine<LivenessContext, LivenessEvent>(
  {
    ...
    states: {
      ...
      start: {
        ...
        always: [{ target: 'detectFaceBeforeStart', cond: 'shouldSkipStartScreen' }],
      ...
      recording: {
        ...
        states: {
          ...
          // If `hasFaceMatchedInOval` is true, then move to `waitForChallengeEvent`, which waits
          // for the HANDLE_CHALLENGE event before proceeding to handleChallenge state.
          // If not, move back to ovalMatching and re-evaluate match state
          checkMatch: {
            entry: () => {},
            after: {
              0: {
                target: 'waitForChallengeEvent',
          ...
          waitForChallengeEvent: {
            entry: () => {},
            invoke: {
              src: 'detectFaceAndMatchOval',
              onDone: {
                target: 'checkChallengeOrContinue',
                actions: 'updateFaceDetailsPostMatch',
              },
            },
            on: {
          ...
          checkChallengeOrContinue: {
            entry: () => {},
            after: {
              100: { target: 'waitForChallengeEvent' },
            },
            on: {
              HANDLE_CHALLENGE: 'handleChallenge',
            },
      ...
      retryableTimeout: {
        ...
        always: [{ target: 'timeout', cond: 'shouldTimeoutOnFailedAttempts' }, { target: 'start' }],
      },
      permissionDenied: {
  ...
  },
  {
    actions: {
      ...
      updateVideoMediaStream: assign({
        videoAssociatedParams: (context, event) => ({
          ...
          videoMediaStream: event.data?.stream as VideoAssociatedParams['videoMediaStream'],
      ...
      updateDeviceAndStream: assign({
        videoAssociatedParams: (context, event) => {
          ...
          context.livenessStreamProvider?.setNewVideoStream(event.data?.newStream as MediaStream)

          return {
            ...context.videoAssociatedParams,
            selectedDeviceId: event.data?.newDeviceId as VideoAssociatedParams['selectedDeviceId'],
            videoMediaStream: event.data?.newStream as VideoAssociatedParams['videoMediaStream'],
          }
        },
      }),
      ...
      startRecording: assign({
        videoAssociatedParams: (context) => {
          if (context.livenessStreamProvider && !context.livenessStreamProvider.isRecording()) {
      ...
      updateFaceMatchBeforeStartDetails: assign({
        faceMatchStateBeforeStart: (_, event) => event.data!.faceMatchState as FaceMatchState,
      }),
      ...
      updateOvalAndFaceDetailsPostDraw: assign({
        ovalAssociatedParams: (context, event) => ({
          ...
          initialFace: event.data!.initialFace as OvalAssociatedParams['initialFace'],
          ovalDetails: event.data!.ovalDetails as OvalAssociatedParams['ovalDetails'],
          scaleFactor: event.data!.scaleFactor as OvalAssociatedParams['scaleFactor'],
        }),
        faceMatchAssociatedParams: (context, event) => ({
          ...context.faceMatchAssociatedParams,
          faceMatchState: event.data!.faceMatchState as FaceMatchAssociatedParams['faceMatchState'],
      ...
      updateErrorStateForRuntime: assign({
        errorState: (_, event) =>
          (event.data?.errorState as ErrorState) || LivenessErrorState.RUNTIME_ERROR,
      }),
      ...
      callUserPermissionDeniedCallback: assign({
        errorState: (context, event) => {
          ...
          const errorMessage = (event.data!.message as ErrorState) || event.data!.Message
    ...
    guards: {
      ...
      isFaceMovementChallenge: (context) => {
        return context.parsedSessionInformation?.Challenge?.Name === FACE_MOVEMENT_CHALLENGE.type
      },
    ...
    services: {
      async checkVirtualCameraAndGetStream(context) {
        ...
        const tracksWithMoreThan15Fps = initialStream.getTracks().filter((track) => {
